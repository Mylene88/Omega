...
async function generateGeoJSON(projet, res) {
  const features = [];
  ...
  res.setHeader('Content-Disposition', `attachment; filename="projet_${projet.id_projet}_complet.geojson"`);
  res.setHeader('Cache-Control', 'no-cache');
  return res.send(jsonBuffer);
}


async function generateCSV(projet, res) {
  const rows = [];
  ...
  res.setHeader('Content-Disposition', `attachment; filename="projet_${projet.id_projet}_complet.csv"`);
  res.setHeader('Cache-Control', 'no-cache');
  return res.send(csvBuffer);
}


async function generatePDF(projet, res) {
  ...
  try {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="projet_${projet.id_projet}.pdf"`);
    res.setHeader('Cache-Control', 'no-cache');
...
export default async function handler(req, res) {
  ...
  try {
    ...
    switch (format.toLowerCase()) {
      case 'pdf':
        return await generatePDF(projetData, res);

      case 'csv':
        return await generateCSV(projetData, res);

      case 'geojson':
        return await generateGeoJSON(projetData, res);
