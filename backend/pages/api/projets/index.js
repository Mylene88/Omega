...
// app_backup/api/projets/route.js


import db from '../../../models';  // ✅ Chemin corrigé
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const result = projets.map(p => ({
      ...
      demande_archivage: p.demande_archivage,
      demande_restauration: p.demande_restauration,
  ...
  try {
    ...
    const securisedData = {
      ...
      demande_archivage: body.demande_archivage,
      demande_restauration: body.demande_restauration,
    ...
    if (isUpdate) {
      ...
      await projetExistant.update({
        ...
        demande_archivage: securisedData.demande_archivage ?? projetExistant.demande_archivage,
        demande_restauration: securisedData.demande_restauration ?? projetExistant.demande_restauration,
        is_archived: securisedData.is_archived ?? projetExistant.is_archived,
        archived_at: securisedData.archived_at ?? projetExistant.archived_at,
        archived_by: securisedData.archived_by ?? projetExistant.archived_by,
  ...
  if (isUpdate) {
    ...
    const { getModelByValue } = require('../../../lib/config');
