...
// backend/app_backup/api/auth/login/route.js
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!isValidPassword) {
      ...
      const { body, status } = errorResponse('Identifiants incorrects', 401);
      console.log('❌ Login échoué pour:', username, '- status:', status);
      console.log('📤 Réponse erreur:', JSON.stringify(body));
      return res.json(body, { status });
    }
    ...
    console.log('✅ Login réussi pour:', username, '- first_login:', user.first_login);
