...
- [ ] Créer le fichier de configuration
  ```bash
  cd frontend
  cp .env.developpement.developpement.example .env.developpement.developpement.local
  ```
...
│   └── index.html
├── backend/                 # API Next.js
└── .env.developpement.developpement.production          # Variables d'environnement
```
...
```bash
# Terminal 1 : Lancer l'app_backup
npm start
